...
export const getUsers = async (req: Request, res: Response, next: NextFunction) => {
    if (req.session.userRole === 'ADMIN') {
        try {
            const foundUsers = await userMongoModel.find({ $rename: { _id: 'id' } }, '-password -__v');
            ...
            if (foundUsers) {
                ...
                return res.status(404).json({ status: 'failed', reason: 'No users found' });
            }
        } catch (error) {
            next(error);
        }
        ...
        try {
            const foundUser = await userMongoModel.findOne({ _id: req.session.userId, $rename: { _id: 'id' } }, '-password -__v');
            if (foundUser) {
