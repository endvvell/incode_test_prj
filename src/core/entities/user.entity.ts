...
export class User {
        ...
        if (inputObj.role) {
            if (inputObj.role.toUpperCase() === 'BOSS') {
                if (!inputObj.subordinates || inputObj.subordinates.length === 0) {
                    ...
                    this.role = this.validUserRole(
                        inputObj.role,
                        inputObj.subordinates,
                    )
                }
            } else {
    ...
    validUserRole(
        ...
        if (
            ...
            throw new InvalidInputError({
                message:
                    "Invalid value for the user's role provided, options - admin, boss, regular",
                statusCode: 400,
            })
        } else {
        ...
        if (typeof value === 'string') {
            ...
            value.forEach((sub_username) => {
                if (sub_username === this.username) {
                    throw new InvalidInputError({
                        message:
                            'Invalid value: a boss cannot be their own subordinate',
                        statusCode: 400,
                    })
                } else {
                    try {
